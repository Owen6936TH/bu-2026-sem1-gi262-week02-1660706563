...
namespace Assignment
{
    public class Assignment : MonoBehaviour
    {
        ...
        public void AS04_AttackEnemy()
        {
            ...
            as04_enemyHP[0] = Math.Max(0, as04_enemyHP[0] - as04_damage);
            Debug.Log($"FirstEnemy hp :{as04_enemyHP[0]}");

            as04_enemyHP[lastIndex] = Math.Max(0, as04_enemyHP[lastIndex] - as04_damage);
            Debug.Log($"LastEnemy hp :{as04_enemyHP[lastIndex]}");

            as04_enemyHP[as04_target] = Math.Max(0, as04_enemyHP[as04_target] - as04_damage);
        ...
        public void EX_01_TicTacToeGame_TurnPlay()
        {
            ...
            bool isOutsideBoard = ex01_row < 0 || ex01_row >= board.GetLength(0) ||
                                  ex01_column < 0 || ex01_column >= board.GetLength(1);

            if (isOutsideBoard || !string.IsNullOrEmpty(board[ex01_row, ex01_column]))
            {
                PrintBoard(board);
            ...
            for (int i = 0; i < 3; i++)
            {
                if ((board[i, 0] == ex01_playerTurn && board[i, 1] == ex01_playerTurn && board[i, 2] == ex01_playerTurn) ||
                    (board[0, i] == ex01_playerTurn && board[1, i] == ex01_playerTurn && board[2, i] == ex01_playerTurn))
                {
                    hasWon = true;
                    break;
                }
            ...
            if (!hasWon)
            {
                hasWon = (board[0, 0] == ex01_playerTurn && board[1, 1] == ex01_playerTurn && board[2, 2] == ex01_playerTurn) ||
            ...
            Debug.Log(isFull ? ">> Draw" : ">> Continue");
        }
        #endregion
